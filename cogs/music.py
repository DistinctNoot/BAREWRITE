...
class Music(commands.Cog):
    ...
    async def loop(self, ctx):
      ...
      if player.repeat == True:
        player.repeat = False
        em = discord.Embed(description="Looping disabled!", color=0xff0000)
        await ctx.message.add_reaction(str('✅'))
        await ctx.send(embed=em)
      else:
        player.repeat = True
        em = discord.Embed(description="Looping enabled!",color=0xff0000)
        await ctx.message.add_reaction(str('✅'))
        await ctx.send(embed=em)
    ...
    async def skip(self, ctx):
      player = self.client.lavalink.player_manager.get(ctx.guild.id)
      await player.skip()
      em = discord.Embed(description="Skipped!", color=0xff0000)
      await ctx.message.add_reaction(str('✅'))
      await ctx.send(embed=em)
